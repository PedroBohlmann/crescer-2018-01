-- Criando ambiente no banco
create tablespace tccdat
datafile 'C:\oraclexe\app\oracle\oradata\XE\tccdat.dbf'
size 100m
autoextend on
next 100m
maxsize 2048m;

create user tcc identified by tcc
default tablespace tccdat;

grant connect, resource, create view to tcc;


-- Criação do banco
CREATE TABLE USUARIO(
    ID_USUARIO NUMBER(10) NOT NULL,
    NOME_COMPLETO VARCHAR2(255) NOT NULL,
    EMAIL VARCHAR2(255) NOT NULL,
    APELIDO VARCHAR2(50),
    DATA_DE_NASCIMENTO DATE NOT NULL,
    SENHA VARCHAR2(128) NOT NULL,
    IMAGEM_DE_PERFIL VARCHAR2(512)
);

ALTER TABLE USUARIO ADD CONSTRAINT USUARIO_PK PRIMARY KEY(ID_USUARIO);

CREATE SEQUENCE USUARIO_SEQ;

CREATE TABLE POST(
    ID_POST NUMBER(10) NOT NULL,
    ID_USUARIO NUMBER(10) NOT NULL,
    TEXTO VARCHAR2(512) NOT NULL,
    VISIBILIDADE VARCHAR2(128) NOT NULL
);
ALTER TABLE POST ADD CONSTRAINT POST_PK PRIMARY KEY(ID_POST);
ALTER TABLE POST ADD CONSTRAINT USUARIO_FK FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO(ID_USUARIO);

CREATE SEQUENCE POST_SEQ;

CREATE TABLE CURTIDA(
    ID_CURTIDA NUMBER(10) NOT NULL,
    ID_USUARIO NUMBER(10) NOT NULL,
    ID_POST NUMBER(10) NOT NULL
);

ALTER TABLE CURTIDA ADD CONSTRAINT CURTIDA_PK PRIMARY KEY (ID_CURTIDA);

ALTER TABLE CURTIDA ADD CONSTRAINT USUARIO_CURTIDA_FK FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO(ID_USUARIO);
ALTER TABLE CURTIDA ADD CONSTRAINT POST_FK FOREIGN KEY (ID_POST) REFERENCES POST(ID_POST);

CREATE SEQUENCE CURTIDA_SEQ;

CREATE TABLE COMENTARIO(
    ID_COMENTARIO NUMBER(10) NOT NULL,
    ID_USUARIO NUMBER(10) NOT NULL,
    ID_POST NUMBER(10) NOT NULL,
    TEXTO VARCHAR2(100) NOT NULL
);

ALTER TABLE COMENTARIO ADD CONSTRAINT COMENTARIO_PK PRIMARY KEY(ID_COMENTARIO);

ALTER TABLE COMENTARIO ADD CONSTRAINT USUARIO_COMENTARIO_FK FOREIGN KEY (ID_USUARIO) REFERENCES USUARIO(ID_USUARIO);
ALTER TABLE COMENTARIO ADD CONSTRAINT POST_COMENTARIO_FK FOREIGN KEY (ID_POST) REFERENCES POST(ID_POST);

CREATE SEQUENCE COMENTARIO_SEQ;


CREATE TABLE CONTATO(
    ID_CONTATO NUMBER(10) NOT NULL,
    ID_USUARIO NUMBER(10) NOT NULL,
    ID_USUARIO_CONVIDADO NUMBER(10) NOT NULL,
    STATUS_CONTATO VARCHAR2(20) NOT NULL
);

ALTER TABLE CONTATO ADD CONSTRAINT CONTATO_PK PRIMARY KEY (ID_CONTATO);

ALTER TABLE CONTATO ADD CONSTRAINT USUARIO_CONTATO_FK FOREIGN KEY(ID_USUARIO) REFERENCES USUARIO(ID_USUARIO);

ALTER TABLE CONTATO ADD CONSTRAINT USUARIO_CONVIDADO_FK FOREIGN KEY(ID_USUARIO_CONVIDADO) REFERENCES USUARIO(ID_USUARIO);

CREATE SEQUENCE CONTATO_SEQ;